...
import site.liuming.wechat.common.util.HttpUtils;
...
public class ApiUrlRequest {
    ...
    public static JSONObject getAppAccessToken(String appId, String appSecret) throws IOException {
        String url = ApiUrlConsts.APP_ACCESS_TOKEN_URL.replace("APPID", appId).replace("APPSECRET", appSecret);
    ...
    public static JSONObject createMenu(String appAccessToken, String menuStr) throws IOException {
        String url = ApiUrlConsts.MENU_CREATE_URL.replace("ACCESS_TOKEN", appAccessToken);
    ...
    public static JSONObject getWebAccessToken(String appId, String appSecret, String code) throws IOException {
        String url = ApiUrlConsts.WEB_ACCESS_TOKEN_URL.replace("APPID", appId).replace("SECRET", appSecret).replace("CODE", code);
        String response = HttpUtils.doGet(url);
        return JSON.parseObject(response);
    }
    ...
    public static JSONObject getJsApiTicket(String appAccessToken) throws IOException {
        String url = ApiUrlConsts.WEB_JSAPI_TICKET_URL.replace("ACCESS_TOKEN", appAccessToken);
        String response = HttpUtils.doGet(url);
        return JSON.parseObject(response);
    }
    ...
    public static String getUnifiedOrderResult(String unifiedOrderXmlStr) throws IOException {
        return HttpUtils.doPost(ApiUrlConsts.PAY_UNIFIED_ORDER_URL, unifiedOrderXmlStr);
    }
